...
int ninjaTraining(int n, vector<vector<int>> &points) {
    ...
    vector<vector<int>> dp(n, vector<int>(4, -1)); // dp of the array of 2 dimensions 
    // Start the recursive calculation from the last day with no previous activity
    return f(n - 1, 3, points, dp); // Recursion approach in details how last and n-1  prepare such that we will be having new values in addition 
    
}

int main() {
    // Define the points matrix
    // define the matrix with the elemetns into it .
    ...
    // calling the ninja training function to find the maximum poitns and print the result 
